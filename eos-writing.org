...
Sometimes I need to unfill paragraphs (the undo of =M-q=):

#+BEGIN_SRC emacs-lisp
(use-package unfill
  :ensure t
  :bind (("M-q" . eos/maybe-unfill-toggle)
         ("M-Q" . unfill-paragraph))
  :config
  (defun eos/maybe-unfill-toggle ()
    "Fill paragraph at or after point (see `unfill-paragraph').
...
(define-skeleton eos/org-wrap-diagram-drawer
  "Wrap text with a :DIAGRAM: / :END: drawer for org-mode"
  nil
  > ":DIAGRAM:" \n
  > _ \n
  > ":END:" \n)
...
(defhydra eos/hydra-skeleton nil
  "Insert Skeleton"
  ("d" eos/org-wrap-diagram-drawer "Wrap as drawer" :exit t)
...
* Plumbing writing on Firefox pages

Sometimes I need to type really long responses in things. There are a couple of nice tools for this.
Both are required for it to work.

First, [[https://github.com/alpha22jp/atomic-chrome][atomic-chrome]] is needed on the Emacs side, and then [[https://github.com/GhostText/GhostText][GhostText]] on the Firefox side.
...
(use-package atomic-chrome
  ...
  :defer 10
  :init
  ;; See: https://github.com/alpha22jp/atomic-chrome/pull/28
