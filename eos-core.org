...
Moving by visual line is far superior when dealing with large bodies of text
that have unbroken lines. So let's enable it everywhere.
...
I restart emacs a lot, especially on my n900, and it would be nice to have the
history of things like =M-x= saved across those sessions. =savehist= mode gives
us that[fn:20:
https://github.com/qdot/conf_emacs/blob/master/emacs_conf.org#global-minor-modes
