...
And now the huge org-mode configuration. I =ensure= the =org-plus-contrib= package so that it
installs not only the latest Org-mode, but also the included contribution packages.
...
I use org-mode's clocking for time management, and mostly to keep track of where the week went for
our weekly emails.
...
(use-package org
  ...
  (setq org-clock-history-length 10
        ...
        ;; If idle for more than 15 minutes, resolve the things
        org-clock-idle-time 15
        ;; some default parameters for the clock report
        org-agenda-clockreport-parameter-plist
        '(:maxlevel 10 :fileskip0 t :score agenda :block thismonth
                    :compact t :narrow 60)))
#+END_SRC
...
I tend to use quite a few different tools from org buffers, so let's install the modes required. In
this case, it's mostly gnuplot and gnuplot-mode for generating graphs of data in org-mode buffers.
