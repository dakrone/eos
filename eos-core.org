...
:CUSTOM_ID: eyebrowse
:END:
Eyebrowse is a great package for workspaces in Emacs. I pre-create 4 workspaces and bind =M-1=, =M-2=, etc to the different workspaces. In this way, I treat it like a window manager, but just for Emacs. Since I use =C-1=, =C-2=, etc for transitioning between XFCE virtual workspaces, the bindings complement each other.
